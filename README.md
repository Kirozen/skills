...
SQLite-backed spec-driven development. The spec lives in a database; `SPEC.md` is a read-only view regenerated on demand by `sdd spec` (spec.db is the sole store).
...
| `sdd-drift` | Read-only detector for code-vs-spec drift; reports violations by severity, writes nothing. A different axis from `sdd spec` (which just re-renders SPEC.md). |
